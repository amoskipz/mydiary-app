...
def initialize(passphrases):
    db.init('diary.db', passphrases=passphrases)
...
if __name__ == '__main__':
    ...
    if not passphrase:
        sys.stderr.write('Password required to access diary.\n')
        sys.stderr.flush()
        sys.exit(1)
    elif len(passphrase) < 8:
        sys.stderr.write('Passphrase must be at least 8 characters long\n')
        sys.stderr.flush()
        sys.exit(1)
